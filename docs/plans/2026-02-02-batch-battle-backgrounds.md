# Batch Battle Background Generator

## Overview

Create a Node.js script to batch generate 600x1000 portrait battle backgrounds for all quest nodes that need them.

## Selection Criteria

A node needs a background if:
1. It has a `battles` array (skip exploration nodes, skip nodes without battles)
2. AND either:
   - No image exists at `src/assets/battle_backgrounds/{nodeId}.png`
   - OR existing image dimensions are not 600x1000

## Prompt Format

```
{node.name}. {node.region}. No labels. RPG battle background. No people. No animals. No monsters. Dark fantasy pixel art. Portrait orientation.
```

Same as `buildDefaultPrompt()` in `src/components/admin/BackgroundAssetModal.vue`.

## Implementation

### File: `scripts/generate-battle-backgrounds.js`

```javascript
#!/usr/bin/env node
/**
 * Batch generate 600x1000 portrait battle backgrounds using Gemini API.
 * Run with: node scripts/generate-battle-backgrounds.js
 *
...
const MODEL = 'gemini-2.0-flash-exp'  // Free tier with image gen
...
```bash
node scripts/generate-battle-backgrounds.js
```

## Notes

- Uses `gemini-2.0-flash-exp` (free tier with image generation)
- Aspect ratio `9:16` is closest to 3:5 (600x1000)
- Sharp resizes to exact 600x1000 with cover fit
- Rate limit handling with automatic retry
- Saves prompts to `assetPrompts.json` with `bg_` prefix
- Idempotent: skips correctly-sized existing images
